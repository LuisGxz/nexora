...
`nexoradevs.com` (apex, canonical), `www.nexoradevs.com` and the legacy `nexora-gye.vercel.app` are all attached to the Vercel project. `www` 308s to the apex via the host-matched rule in `vercel.json` (path preserved). The `.vercel.app` alias does **not** redirect — Vercel does not apply `has: host` redirects to its own domains — so it serves the site directly; that is deliberate, since the owner shared that link during outreach and a 404 would be worse than a duplicate. Its `<link rel="canonical">` points at the apex, so search engines still consolidate on one URL.

DNS lives at **Cloudflare** — the apex and `www` are `A` records to `76.76.21.21`, set to **DNS only (grey cloud)**: proxying them breaks Vercel's certificate issuance.
